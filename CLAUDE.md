...
```
app/        应用层 + 部分业务层（banyuntask.c、NavigationMecanum.c、BollLocator.c、Mecanum_Move.c）
algorithm/  业务层（Nav_position.c 轮式里程计、mecanum.c）
hardware/   硬件驱动层（hwt_imu.c、pid.c、k230.c、wit_protocol.c、oled.c …）
device/     抽象设备层 —— 接口 + locator_wheel 实现（OPS9/光流尚未接入）
Core/       CubeMX 生成（main.c、app_freertos.c、外设初始化）
uart/       msp_uart2.c
obsolete/   已停用的驱动（imu660/、hwt101_legacy/），不在任何 CMake glob 内，不参与编译
```
